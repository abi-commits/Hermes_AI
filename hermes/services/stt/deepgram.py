...
class DeepgramSTTService(AbstractSTTService):
    ...
    def _extract_transcript(message: Any) -> str:
        ...
        try:
            # Check if message is the LiveResultResponse object (v3 SDK)
            ...
            if isinstance(message, dict):
                return message.get("channel", {}).get("alternatives", [{}])[0].get("transcript", "").strip()
            
            return ""
        except (AttributeError, IndexError, KeyError):
            return ""

    def _consume_result(self, message: Any, finalized_segments: list[str]) -> str | None:
    ...
    def _live_transcription_options(self) -> dict[str, Any]:
        ...
        return {
            ...
            # REMOVED: utterance_end_ms (Caused 400 rejection in mulaw mode)
            "encoding": "mulaw",
            "sample_rate": 8000,
            "channels": 1,
        }
    ...
    async def transcribe(self, audio: np.ndarray) -> str:
        """Transcribe via pre-recorded API (Linear16)."""
        if self._client is None:
            await self.connect()
        # Non-streaming implementation for non-voice tasks
        return "Not implemented for pure array"
    ...
    async def stream_transcribe(
        ...
        async def _on_message(_sender, result, **kwargs) -> None:
            utterance = svc._consume_result(result, finalized_segments)
            if utterance:
                await transcript_queue.put(utterance)

        async def _on_speech_started(_sender, speech_started, **kwargs) -> None:
            svc._logger.debug("deepgram_speech_started")
            await transcript_queue.put(InterruptMarker())

        async def _on_error(_sender, error, **kwargs) -> None:
            svc._logger.error("deepgram_live_error", error=str(error))
